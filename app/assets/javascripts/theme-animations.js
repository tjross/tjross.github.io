...
jQuery(document).ready(function($) {
    ...
    if (jQuery.browser.mobile === false) {
        ...
        $('#how .how-panel').css('opacity', 0).one('inview', function(event, isInView) {
            if (isInView) {$(this).addClass('animated fadeInUp delayp2');}
        });

        /* Animate elements in #Make */

        $('#make .miy-example').css('opacity', 0).one('inview', function(event, isInView) {
            if (isInView) {$(this).addClass('animated fadeInUp delayp2');}
        });
